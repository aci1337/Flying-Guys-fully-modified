...
bool IsRatDown(int vk)
{
	mouse_interface();
	return (ilikewhenipee(vk) & 0x8000) != 0;
}
...
void KeyboardShortcuts()
{
	while (true)
	{
		...
		if (ilikewhenipee(VK_HOME))
			maxspeed_multiplier += 10;

		if (ilikewhenipee(VK_END))
			maxspeed_multiplier -= 10;

		if (ilikewhenipee(VK_PRIOR)) // Page Up
			gravity_multiplier += 10;

		if (ilikewhenipee(VK_NEXT)) // Page Down
			gravity_multiplier -= 10;
...
int main()
{
	...
	std::cout << "Waiting for Fall Guys:)";

	while (hwnd == NULL)
	{
		hwnd = FindWindowA(0, ("FallGuys_client"));
		Sleep(100);
	}
